"""T4.combined.nn — tuned PyTorch MLP for the `combined` dataset (delay_s regression).

Run from this folder:
    MC_SMOKE=1 MC_THREADS=2 python nn_combined.py    (smoke, <5 min)
    MC_THREADS=2 python nn_combined.py                (full run, T5 launches this)
...
# This file lives at models/combined/nn_combined.py -> parents[2] is model_comparison/, per protocol.
...
print(f"  {'dataset':<8} {'family':<3} {'model':<4} {'r2':>8} {'mae':>8} {'rmse':>8}")
print(f"  {DS:<8} {'nn':<3} {'MLP':<4} {test_metrics['r2']:>8.4f} {test_metrics['mae']:>8.1f} {test_metrics['rmse']:>8.1f}")
